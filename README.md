...
- **핵심 도메인 (비콘·장소·문서)** — 비콘 일생주기·장소 계층 CRUD·문서 관리를 ENT-BE에서 구현
- **DBMS 메타 트리 API** — 여러 번 부르던 API를 트리 단일 호출로 통합, 항목별 차트 표시 가능 여부 판단을 백엔드로 일원화
- **인증 서버 재구현** — 기존 인증 서버의 유저·프로젝트·부서·초대·문의 도메인을 NestJS로 재구현, 컨벤션·검증·기존 API 호환 담당
...
- **피드백을 새 시도로** — 사내 사용 팀의 불편을 듣고 문서 뷰어 개선을 직접 기획·PR
